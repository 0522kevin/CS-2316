...
array([11.99  15.5  7.75  20.49])
	"""
	return cart[(cart <= item_budget) & (cart != 17.5)]
...
array([32. 32.  8.  0. 15. 32.])
	"""
	return np.where(labels == "High", float(32), np.where(labels == "Mid", float(15), np.where(labels == "Low", float(8), np.where(labels == "Insufficient", float(0), float(0)))))
