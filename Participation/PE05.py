...
def script_searcher(keyword, target):
    ...
    'bed bug'
    """
    return re.search(rf"\w+ {keyword}", target).group()
...
def double_l(target):
    ...
    """
    return re.findall(r"\s*(\w*ll\w*)\s*", target)
...
def quotable(target, character):
    ...
    Each line of dialogue will end in some kind of punctuation, ?,!, or .
    ...
    """
    return re.findall(rf"{character}: ([^?!.]*[?!.]?)", target)[0]
