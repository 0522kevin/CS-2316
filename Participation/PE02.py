...
class Podcast:
    ...
    def __eq__(self, other):
        ...
        Question 1b
            ...
            Example:
                ...
                True
        """
        return True if self.name == other.name and self.total_episodes == other.total_episodes else False
    ...
    def __lt__(self, other):
        ...
        Question 1c
            ...
            Example:
                ...
                False
        """
        return True if self.total_episodes < other.total_episodes else False
    ...
    def __repr__(self):
        ...
        """
        return f"The \"{self.name}\" podcast, hosted by " + " and ".join(self.host_names) + f", has {self.total_episodes} episodes and is in the {self.genre} category."
...
def category_podcast(podcast_list, category):
    ...
    """
    return [podcast.name for podcast in podcast_list if podcast.genre == category]
...
def assign_logos(logos, podcast_list):
    ...
    """
    return {podcast.name : logo for logo, podcast in zip(logos, podcast_list)}
...
def bad_host(podcast, removal_host):
    ...
    Question 4 (solution must be written in one line)
        ...
        Example:
            ...
            ['Ashley Flowers']
    """
    podcast.host_names = [host for host in podcast.host_names if host != removal_host]
...
def episode_updater(original_podcasts):
    ...
    Question 5 (solution can be written using multiple lines)
        ...
        Example:
            ...
            True
    """
    temp_podcasts = copy.deepcopy(original_podcasts)
    for podcast in temp_podcasts:
        podcast.total_episodes += 1
    return temp_podcasts
...
if __name__ == "__main__" :
    ...
    pod1 = Podcast("Crime Junkies", "True Crime", 453, ['Ashley Flowers', 'Brit Prawat'])
    pod2 = Podcast("Crime Junkies", "Science Fiction", 453, ['audiochuck'])
    # print(pod1 == pod2) # True

    ### test __lt__ method ###
    pod3 = Podcast("Freakonomics Radio", "Education", 490, ['Stephen J. Dubner'])
    pod4 = Podcast("Dear Hank & John", "Philosophy", 316, ['Hank Green', 'John Green'])
    # print(pod3 < pod4) # False

    ### test __repr__ method ###
    pod5 = Podcast("The Journal", "News", 847, ['Kate Linebaugh', 'Ryan Knutson'])
    # print(pod5) # The "The Journal" podcast, hosted by Kate Linebaugh and Ryan Knutson, has 847 episodes and is in the News category
    ...
    pod_list = [p1, p2, p3]
    # print(category_podcast(pod_list,'Business')) # ['Psychedelics for the Win', 'Exponential Organization']
    p_list2 = [pod1,pod2,pod4]
    # print(category_podcast(p_list2,'Science Fiction')) # ['Crime Junkies']
    ...
    smokingtire = Podcast("The Smoking Tire", "Comedy", 600, ["Matt Farah", "Zack Klapman"])
    podcast_list = [cartalk, pastgas, smokingtire]
    # print(assign_logos(logos, podcast_list)) # {'Car Talk': 'Wrench', 'Past Gas': 'Race Helmet', 'The Smoking Tire': 'Flat Tire'}

    ## Question 4 ###
    # print(pod1.host_names) # ['Ashley Flowers', 'Brit Prawat']
    # bad_host(pod1, 'Brit Prawat')
    # print(pod1.host_names) # ['Ashley Flowers']
    

    ### Question 5 ###
    original_podcasts = [pod1, pod2, pod3]
    new_podcasts = episode_updater(original_podcasts)
    print(original_podcasts[0].total_episodes == new_podcasts[0].total_episodes) # False
    print(original_podcasts[0].total_episodes + 1 == new_podcasts[0].total_episodes) # True
    print(original_podcasts[1].total_episodes + 1 == new_podcasts[1].total_episodes) # True
    print(original_podcasts[2].total_episodes + 1 == new_podcasts[2].total_episodes) # True
