...
def read_csv(input_file):
	...
	with open(input_file, "r", encoding = "utf8") as fin:
		...
		data_list = [["YEAR", reader_list[0][1], reader_list[0][2], reader_list[0][4], reader_list[0][5], reader_list[0][6], reader_list[0][7]]]
		for read in reader_list[1:]:
			data_list.append([int("".join([integer for integer in read[0] if integer.isdigit()])), read[1], read[2] if read[2] != "" else "N/A", read[4], int("".join([integer for integer in read[5] if integer.isdigit()])), int("".join([integer for integer in read[6] if integer.isdigit()])), int("".join([integer for integer in read[7] if integer.isdigit()]))])
		return data_list
...
def export_json(cleaned_list, output_file):
  ...
  2.  Using the extracted list from `read_csv()`, create a dictionary that maps each movie name to an
	...
	"""
	dic = {}
	for clean in cleaned_list[1:]:
		dic[clean[1]] = {"Average Ticket Price": round(clean[4]/clean[6], 2), "Genre": clean[2], "Year": clean[0]}
...
3.	Using the cleaned list of lists from question 1, find the difference in total profit in 2019$ 
	...
	'''
	first = sorted(cleaned_list[1:], key = lambda x : x[5])[-1][1]
	first_sales = sorted(cleaned_list[1:], key = lambda x : x[5])[-1][5]
	second = sorted(cleaned_list[1:], key = lambda x: x[5])[-2][1]
	second_sales = sorted(cleaned_list[1:], key = lambda x : x[5])[-2][5]
	return f"The highest grossing film, {first}, earned ${first_sales - second_sales} more than {second}."
...
4. 	You and your friends are having a movie night, but you keep running into
	...
	'''
	movie_list = [clean for clean in cleaned_list[1:] if (clean[3] != "Walt Disney" and clean[3] != "Warner Bros.") if (clean[2] == "Action" or clean[2] == "Adventure") if clean[0] % 2 == 0 if clean[1] != "Shrek 2"]
	movie_list.sort(key = lambda x : x[6], reverse = True)
	with open(output_file, "w") as fout:
		writer = csv.writer(fout, quoting = csv.QUOTE_NONE, escapechar = "\b")
		# writer = csv.writer(fout)
		for movie in movie_list:
			fout.write(f"{movie[1]}, released in {movie[0]}, was produced by {movie[3]} and grossed ${movie[5]} in 2019 dollars.\n")
			# writer.writerow([f"{movie[1]}, released in {movie[0]}, was produced by {movie[3]} and grossed ${movie[5]} in 2019 dollars."])
			# writer.writerow([str(movie[1]) + ", released in " + str(movie[0]) + ", was produced by " + str(movie[3]) + " and grossed $" + str(movie[5]) + " in 2019 dollars."])
	return movie_list
